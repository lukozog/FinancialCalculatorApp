...
public class UserSavingsGoalsService {
    ...
    public LocalDate savingsGoalDateByDailySavings(long userId, long savingsGoalId, double dailySavings) {
        if (savingsGoalDao.getSavingsGoal(userId, savingsGoalId).isPresent()) {
            ...
            return LocalDate.now().plusDays(howManyDaysToAccomplish);
        }
        return null;
    }

    public LocalDate savingsGoalDateByMonthlySavings(long userId, long savingsGoalId, double monthlySavings) {
        return savingsGoalDateByDailySavings(userId, savingsGoalId, monthlySavings / LocalDate.now().lengthOfMonth());
    }

    public Double savingsGoalDailySavingTillDate(long userId, long savingsGoalId, LocalDate dateOfAccomplishment) {
        if (savingsGoalDao.getSavingsGoal(userId, savingsGoalId).isPresent()) {
            long daysBetween = DAYS.between(LocalDate.now(), dateOfAccomplishment);
            double savingsGoalMoneyToAccomplish = savingsGoalDao.getSavingsGoal(userId, savingsGoalId).get().getSavingsGoalMoneyToAccomplish();
