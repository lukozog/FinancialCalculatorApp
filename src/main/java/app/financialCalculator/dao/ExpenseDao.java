...
import app.financialCalculator.util.LocalDateConverter;
...
public class ExpenseDao {
    ...
    private Field<LocalDate> dateField= DSL.field("EXPENSES.EXPENSE_DATE", SQLDataType.DATE.asConvertedDataType(new LocalDateConverter()));


    public void createExpense(Expense expense) {
        dslContext.insertInto(EXPENSES, EXPENSES.EXPENSE_NAME, EXPENSES.EXPENSE_DESCRIPTION,
                EXPENSES.EXPENSE_VALUE, dateField,
    ...
    public void updateExpense(Expense expense, long expenseId) {
        dslContext.update(EXPENSES)
                ...
                .set(dateField, expense.getExpenseDate())
    ...
    public List<Expense> getExpenses(long userId, LocalDate minDate, LocalDate maxDate, long savingsTypeId) {
        return dslContext.selectFrom(EXPENSES)
                .where(EXPENSES.EXPENSE_USER_ID.eq(userId))
                .and(dateField.between(minDate, maxDate))
                .and(EXPENSES.EXPENSE_TYPE_ID.notEqual(savingsTypeId))
                .fetchInto(Expense.class);
    }

    public List<Expense> getExpenses(long userId, LocalDate minDate, LocalDate maxDate, long expenseTypeId, long savingsTypeId) {
        return dslContext.selectFrom(EXPENSES)
                .where(EXPENSES.EXPENSE_USER_ID.eq(userId))
                .and(dateField.between(minDate, maxDate))
    ...
    public List<Expense> getUserExpensesForMonth(long userId, LocalDate firstDay, LocalDate lastDay, long savingsTypeId) {
        return dslContext.selectFrom(EXPENSES)
                .where(EXPENSES.EXPENSE_USER_ID.eq(userId))
                .and(dateField.between(firstDay, lastDay))
                .and(EXPENSES.EXPENSE_TYPE_ID.notEqual(savingsTypeId))
                .fetchInto(Expense.class);
    }
